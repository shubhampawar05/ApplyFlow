...
export async function matchApplicationForUser(userId: string, applicationId: string) {
  ...
  const resume =
    application.resume ??
    (await getDefaultResumeWithProfile(userId));
  ...
  const jobInput = toJobMatchInput(application.job);
  const resumeInput = toResumeMatchInput(resume.profile);
